        ...
        if([self getDistance:touch1.x wiht:touch1.y with:touch2.x with:touch2.y] > 50)
            if( touch2.x != 0 && touch2.y != 0){  /////////////////防止多點觸迫bug
                ...
                if(touch1.x>touch2.x && touch1.y>touch2.y )
                    Object = [[UMLclass alloc] initType:4 with:currentview.ObjectList.count x1:touch2.x andy1:touch2.y andx2:touch1.x andy2:touch1.y and:port];
                if(touch1.x>touch2.x && touch1.y<touch2.y )
                    Object = [[UMLclass alloc] initType:4 with:currentview.ObjectList.count x1:touch2.x andy1:touch1.y andx2:touch1.x andy2:touch2.y and:port];
                if(touch2.x>touch1.x && touch2.y>touch1.y )
                    Object = [[UMLclass alloc] initType:4 with:currentview.ObjectList.count x1:touch1.x andy1:touch1.y andx2:touch2.x andy2:touch2.y and:port];
                if(touch2.x>touch1.x && touch2.y<touch1.y )
                    Object = [[UMLclass alloc] initType:4 with:currentview.ObjectList.count x1:touch1.x andy1:touch2.y andx2:touch2.x andy2:touch1.y and:port];
